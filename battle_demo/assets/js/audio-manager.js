...
class AudioManager {
    ...
    async play(event, context = {}) {
        ...
        // Normalize path - keep relative paths as-is for standalone demo
        // Don't add /public/ prefix - paths should be relative to battle_demo folder
        if (filePath.startsWith('/public/')) {
            // Remove /public/ prefix for standalone demo
            filePath = filePath.replace('/public/', '');
        } else if (filePath.startsWith('/assets/')) {
            // Remove leading slash to make it relative
            filePath = filePath.substring(1);
        }
        // If it already starts with 'assets/', keep it as-is (relative path)
