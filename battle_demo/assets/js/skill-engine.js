...
class SkillEngine {
    ...
    static recalculateStats(entity) {
        ...
        // --- Apply Equipment Bonuses (Static/Persistent) ---
        if (entity.equipmentAttributes) {
            Object.keys(entity.equipmentAttributes).forEach(stat => {
                if (attributes[stat] !== undefined) {
                    attributes[stat] += entity.equipmentAttributes[stat];
                }
            });
        }
        if (entity.equipmentStats) {
            if (entity.equipmentStats.atk) stats.atk += entity.equipmentStats.atk;
            if (entity.equipmentStats.matk) stats.matk += entity.equipmentStats.matk;
            if (entity.equipmentStats.hit) stats.hit += entity.equipmentStats.hit;
            if (entity.equipmentStats.flee) stats.flee += entity.equipmentStats.flee;
            if (entity.equipmentStats.crit) stats.crit += entity.equipmentStats.crit;
        }

        // --- Apply all active buffs (Temporary) ---
