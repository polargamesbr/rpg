...
<div id="combat-modal" class="fixed inset-0 z-[200] hidden opacity-0 transition-opacity duration-500" aria-hidden="true" style="font-family: 'Inter', sans-serif;">
    ...
    <!-- AUDIO -->
    <audio id="audio-bass" src="<?= asset('mp3/bass.mp3') ?>" preload="auto"></audio>
    <audio id="audio-battle-begin" src="<?= asset('mp3/battle-begin.mp3') ?>" preload="auto"></audio>
    <audio id="audio-battle" src="<?= asset('mp3/battle.mp3') ?>" loop preload="auto"></audio>
    <audio id="audio-impact" src="<?= asset('mp3/impact.mp3') ?>" preload="auto"></audio>
    <audio id="audio-parry" src="<?= asset('mp3/sword2.mp3') ?>" preload="auto"></audio>
    <audio id="audio-crit-hit" src="<?= asset('mp3/impact.mp3') ?>" preload="auto"></audio>
    
    <audio id="audio-sword1" src="<?= asset('mp3/sword1.mp3') ?>" preload="auto"></audio>
    <audio id="audio-sword2" src="<?= asset('mp3/sword2.mp3') ?>" preload="auto"></audio>
    <audio id="audio-sword3" src="<?= asset('mp3/sword3.mp3') ?>" preload="auto"></audio>
    <audio id="audio-sword4" src="<?= asset('mp3/sword4.mp3') ?>" preload="auto"></audio>
    ...
    <div class="absolute inset-0 bg-[#050505] transition-colors duration-1000" id="combat-bg-layer">
        <!-- Main BG Image -->
        <img id="combat-bg-img" src="<?= asset('img/battle.webp') ?>" class="absolute inset-0 w-full h-full object-cover object-center opacity-100 blur-[0px] scale-105 transition-transform duration-[20s] ease-linear">
    ...
    <div id="turn-sequence-ui" class="fixed top-4 left-1/2 -translate-x-1/2 z-[199] pointer-events-none opacity-0 -translate-y-full transition-all duration-700 ease-out">
        <div class="relative">
            <!-- Semicircular Arc Container (Barriga para baixo) -->
            <div class="relative w-[600px] h-[120px] flex items-center justify-center">
                <!-- Liquid Glass Background (Inverted) -->
                <div class="absolute inset-0 bg-gradient-to-b from-black/40 via-black/30 to-black/40 backdrop-blur-2xl rounded-3xl border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.4)]" style="clip-path: ellipse(100% 60% at 50% 100%);">
                    <!-- Inner glow -->
                    <div class="absolute inset-0 bg-gradient-to-t from-white/5 via-transparent to-transparent rounded-3xl"></div>
                    <!-- Neomorphism effect -->
                ...
                <!-- Semicircular Arc Path for Avatars (Inverted - barriga para baixo) -->
                <svg class="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 600 120" preserveAspectRatio="none">
                    <defs>
                        <linearGradient id="arcGradient" x1="0%" y1="0%" x2="100%" y2="0%">
                            <stop offset="0%" style="stop-color:rgba(59,130,246,0.3);stop-opacity:1" />
                            <stop offset="50%" style="stop-color:rgba(168,85,247,0.3);stop-opacity:1" />
                            <stop offset="100%" style="stop-color:rgba(59,130,246,0.3);stop-opacity:1" />
                        </linearGradient>
                    </defs>
                    <!-- Arc path (semicircle invertido - barriga para baixo) -->
                    <path id="turn-arc-path" d="M 50 20 Q 300 100 550 20" fill="none" stroke="url(#arcGradient)" stroke-width="2" opacity="0.3"/>
                </svg>
                
                <!-- Avatar Circles Container (positioned along inverted arc) -->
                <div id="turn-sequence-arc" class="absolute inset-0 flex items-center justify-center">
                    <!-- Avatars will be positioned dynamically by JS along the inverted arc -->
                </div>
            </div>
        </div>
    </div>

    <!-- Audio Controls (Floating circular buttons, top-right) -->
    <div class="fixed top-4 right-4 z-[199] flex flex-col gap-2 pointer-events-none">
        <button id="btn-toggle-music" onclick="combatSystem.toggleMusic()" class="w-11 h-11 rounded-full bg-black/70 hover:bg-black/85 backdrop-blur-xl border-2 border-amber-500/40 hover:border-amber-500/60 transition-all flex items-center justify-center group shadow-lg pointer-events-auto hover:scale-110 active:scale-95">
            <i data-lucide="volume-2" class="w-5 h-5 text-amber-400 group-hover:text-amber-300 transition-colors"></i>
        </button>
        <button id="btn-toggle-sfx" onclick="combatSystem.toggleSFX()" class="w-11 h-11 rounded-full bg-black/70 hover:bg-black/85 backdrop-blur-xl border-2 border-blue-500/40 hover:border-blue-500/60 transition-all flex items-center justify-center group shadow-lg pointer-events-auto hover:scale-110 active:scale-95">
            <i data-lucide="volume-1" class="w-5 h-5 text-blue-400 group-hover:text-blue-300 transition-colors"></i>
        </button>
    </div>

    <!-- Active Target Chip (Floating, when needed) -->
    <div id="active-target-chip" class="fixed top-4 left-[140px] z-[199] hidden opacity-0 translate-y-[-10px] transition-all duration-300 pointer-events-none">
        <div class="flex items-center gap-2 px-3 py-1.5 bg-red-500/20 backdrop-blur-xl border border-red-500/30 rounded-full shadow-lg">
            <i data-lucide="crosshair" class="w-3.5 h-3.5 text-red-400 animate-pulse"></i>
            <span id="target-name-display" class="text-[10px] font-black text-white uppercase tracking-wider">Target</span>
        </div>
    </div>
    ...
    <div id="combat-cinema-wrapper" class="absolute inset-0 z-10 flex flex-col pointer-events-none transform transition-transform duration-300 origin-center">
        <div class="flex-1 w-full h-full flex items-center justify-center relative pt-[140px] pb-36" id="battlefield-container">
        ...
        <footer class="fixed bottom-0 left-0 right-0 z-[200] h-32 bg-gradient-to-t from-black via-black/95 to-transparent flex flex-col items-center justify-center pointer-events-none pb-4">
            <div class="pointer-events-auto relative">
                ...
                <div id="action-bar" class="flex items-center gap-6 px-10 py-4 rounded-2xl bg-[#0a0a0a]/80 backdrop-blur-3xl border border-white/10 shadow-[0_25px_60px_rgba(0,0,0,0.9)] ring-1 ring-white/5 transition-all duration-700 ease-out transform translate-y-8 opacity-0 scale-95">
    ...
    <link rel="stylesheet" href="<?= asset('css/item-cards.css') ?>">
    <link rel="stylesheet" href="<?= asset('css/action-bar-visibility.css') ?>">
